...
Python 3.12 recomendado. Los modelos `.keras` incluidos fueron exportados con Keras 3
y se han validado con TensorFlow 2.16.x / Keras 3.x.

## 2. Artefactos locales (`artifacts/`)
...
La probabilidad calibrada (Platt) y el umbral se ajustan en Validation400. La API los
re-ajusta al arrancar desde `external_validation_results.csv`, reproduciendo la
corrección afín del vCDR, el score `0.78·vCDR + 0.22·rCDR`, Platt y el umbral de
sensibilidad objetivo. Los parámetros se cachean en `artifacts/calibration_params.json`.

## 3. Ejecución

Para probar la interfaz HTML sin cargar los modelos pesados, arranca la API en modo demo:
...
Para ejecutar el pipeline real con los artefactos de `artifacts/`:
...
El backend también sirve `frontend/index.html` desde `GET /`, de modo que no hace falta
levantar un servidor aparte para el HTML.
